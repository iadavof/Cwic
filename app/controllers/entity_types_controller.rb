...
private
  def load_resource
    case params[:action]
    when 'index'
      @entity_types = @organisation.entity_types.accessible_by(current_ability, :index).includes(:icon).ssp(params)
    when 'new', 'create'
  ...
  def resource_params
    params.require(:entity_type).permit(
      :name, :description, :icon_id, :slack_before, :slack_after, :min_reservation_length, :max_reservation_length,
