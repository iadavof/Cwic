class Users::InvitationsController < Devise::InvitationsController
  ...
  def new
    ...
    if params[:email].present? && params[:organisation].present?
      selected_organisation = Organisation.find(params[:organisation])
      if selected_organisation.present?
        self.resource.organisation_users.build(organisation: selected_organisation)
  ...
  def configure_permitted_parameters
    devise_parameter_sanitizer.for(:invite) { |u| u.permit(:first_name, :infix, :last_name, :email, :invitation_token, organisation_users_attributes: [:organisation_id, :organisation_role_id]) }
  end
